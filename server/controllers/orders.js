			...
			Order.update({order_number: req.body.order_number}, req.body, function(err){
				if (err){
					...
					for (each in total_items) {
						html_string = html_string + "<tr><td>" + total_items[each]._name + "</td> <td>" + total_items[each]._quantity + " </td> <td>$" + total_items[each]._price.toFixed(2) + "</td></tr>"
					}
